// src/components/NutritionCalculator/NutritionCalculator.jsx
...
const NutritionCalculator = ({ isFloating = false }) => {
  ...
  return (
    <div className={isFloating ? "nutrition-page floating-calculator-content" : "nutrition-page"}>
      <h2 className="title">Nutrition Analyzer</h2>
      <p className="subtitle">Enter your food name to get the nutrition information</p>
 
      <div 
        className={isFloating ? "main-content floating-calculator-main-content" : "main-content"}
      >
        <div 
            className={isFloating ? "left-section floating-calculator-left-section" : "left-section"}
        >
        ...
        <div className="right-section">
          <h3 className="chart-title">Calorie Breakdown (Total: {Math.round(nutrition.calories)} kcal)</h3>
              ...
              <div className="legend">
                {pieData.map((entry, index) => (
                  <div key={entry.name} className="legend-item">
                    ...
                    <span>{entry.name} ({entry.value.toFixed(1)} g)</span>
                  </div>
                ))}
              </div>
          ...
          {showChart && pieData.length === 0 && <p>No valid macro data found for the items entered.</p>}
        </div>
      </div>
    </div>
  );
